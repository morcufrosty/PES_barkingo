            ...
            for (let i = 0; i < chatResponse.length; i++) {
                ...
                if (user) {
                    ...
                    if (responseUser.success && responseOffer.success) {
                        chatAux[i] = { name: responseUser.user.username, desc: strings('chat.interested', {n: responseOffer.offer.name }), type: 'user', chatInfo: chatResponse[i], currentUser: currentUser.user.id, id:chatUserId };
                    }
                } else if (offer) {
                    responseOffer = await this.getOfferInfoFromAPI(tokenJson, chatOfferId);
                    responseUser = await this.getUserInfoFromAPI(tokenJson, chatUserId);
                    ...
                    if (responseOffer.success && responseUser.success) {
                        chatAux[i] = { name: responseOffer.offer.name, desc: strings('chat.interested', {n: responseUser.user.username  }), chatInfo: chatResponse[i], currentUser: currentUser.user.id, id:chatOfferId };
